...
import { space, Input } from '@chakra-ui/react';
import {
   Box,
    Heading,
    Text,
  ...
  return (
    ...
    <Flex justifyContent="space-between">
      ...
      <Box>
          ...
          <Button colorScheme="red" size="sm" onClick={()=>{deleteBlog(item._id)}}>
            Delete
          </Button>
        </Box>
</Flex>
      ...
      <Flex justifyContent={"space-between"}>
      <Heading as="h2" size={"xl"}  color="red" fontSize="sm" fontWeight={"bold"} mt={2}>
        Likes: {item.likes}
      </Heading>
      ...
      <Input type="text" placholder="Type your comment" value={comment} onChange={(e)=>{setComment(e.target.value)}}/>
      <Button bg="orange.500" onClick={()=>{handleComment(item._id)}}>Comment</Button>
      </Flex>
      <Flex justifyContent="space-between">
      <Box mt={4}>
        <Text fontWeight="bold">Comments:</Text>
        {item.comments.map((comment, index) => (
          <Box key={index} mt={2} borderTop="1px" borderColor="gray.300" pt={2}>
            <Text>
